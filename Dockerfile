...
RUN cd /root && apk upgrade --no-cache && \
  apk add --no-cache bash make git fontconfig woff2 file
RUN  wget -q https://github.com/typst/typst/releases/download/v0.13.1/typst-x86_64-unknown-linux-musl.tar.xz && \
